...
function DateDivider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3 bg-[var(--c-paper)] px-4 py-2 border-b border-[var(--c-fill)]">
      <span className="text-xs font-semibold text-[var(--c-mut)] tracking-wide">{label}</span>
      <div className="flex-1 h-px bg-[var(--c-hair)]" />
    </div>
  );
}
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      {icon ? (
        <div className="mb-4 text-[var(--c-faint)]">{icon}</div>
      ) : (
        <svg
          className="mb-4 h-12 w-12 text-[var(--c-faint)]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      <h3 className="text-lg font-medium text-[var(--c-ink)]">{title}</h3>
      {description ? (
...
function MessageSkeleton({ compact = false }: { compact?: boolean }) {
  if (compact) {
    return (
      <div className="flex items-center gap-3 border-b border-[var(--c-fill)] px-3 py-2">
  ...
  return (
    <div className="flex items-center gap-3 border-b border-[var(--c-fill)] px-4 py-3">
...
export function MessageList({
  ...
  if (isLoading) {
    return (
      <div className={cn('divide-y divide-[var(--c-fill)]', className)}>
