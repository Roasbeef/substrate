    ...
    <>
      ...
      <div
        className={cn(
          ...
          'rounded-xl bg-[var(--c-card)] shadow-2xl',
          className,
        )}
      >
        {/* Sticky header with back button and agent info. */}
        <div className="shrink-0 border-b border-[var(--c-hair)] bg-[var(--c-card)] px-8 py-5">
          <div className="flex items-center justify-between">
            <button
              onClick={onBack}
              className="inline-flex items-center gap-1.5 text-sm text-[var(--c-mut)] hover:text-[var(--c-ink)] transition-colors"
            >
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-md border border-[var(--c-hair)] bg-[var(--c-card)] px-3 py-1.5 text-sm font-medium text-[var(--c-ink)] hover:bg-[var(--c-hover)] focus:outline-none focus:ring-2 focus:ring-[#22262A]/30 focus:ring-offset-2"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-8 pt-8 pb-6 text-center">
            ...
            <h2 className="text-2xl font-bold text-[var(--c-ink)]">{agent.name}</h2>
            {getAgentContext(agent) ? (
              <p className="mt-1 text-sm text-[var(--c-mut)]">{getAgentContext(agent)}</p>
            ) : null}
            {agent.git_branch ? (
              <p className="mt-1 flex items-center justify-center gap-1 text-xs text-[var(--c-faint)]">
            ...
            <div className="mt-2 flex items-center justify-center gap-3 text-sm text-[var(--c-mut)]">
          ...
          {summaryLoading ? (
            <div className="mx-8 mb-6 rounded-lg border border-[var(--c-fill)] bg-[var(--c-hover)] p-5">
              <div className="space-y-2">
                <div className="h-4 w-full animate-pulse rounded bg-[var(--c-hair)]" />
                <div className="h-4 w-3/4 animate-pulse rounded bg-[var(--c-hair)]" />
              </div>
            </div>
          ) : summary ? (
            <div className="mx-8 mb-6 rounded-lg border border-[#33608D]/15 bg-[#33608D]/5 p-5">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 shrink-0 rounded-full bg-[#33608D]/10 p-1.5">
                  <svg className="h-4 w-4 text-[var(--c-steel)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-[var(--c-ink)]">Current Activity</p>
                  <p className="mt-1 text-sm text-[var(--c-ink)] leading-relaxed">
                    {summary.summary}
                  </p>
                  {summary.delta && summary.delta !== 'Initial summary' ? (
                    <div className="mt-2 flex items-start gap-1.5">
                      <span className="text-xs font-bold text-[var(--c-steel)]">&#916;</span>
                      <p className="text-xs text-[var(--c-mut)]">{summary.delta}</p>
                    </div>
                  ) : null}
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    {summary.is_stale ? (
                      <span className="inline-flex items-center gap-1 text-xs text-[var(--c-steel)]">
