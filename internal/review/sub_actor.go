...
func (r *reviewSubActor) Run(parentCtx context.Context) {
	...
	prompt := r.buildReviewPrompt(ctx)
...
// buildReviewPrompt loads the diff that was captured at review-request time
// from the database and embeds it directly in the reviewer's prompt. The
// diff is no longer regenerated by the reviewer running git locally — the
// substrate pod doesn't necessarily have the agent's repo on disk.
func (r *reviewSubActor) buildReviewPrompt(ctx context.Context) string {
	diffContent, diffCommand := "", ""
	rev, err := r.store.GetReview(ctx, r.reviewID)
	if err != nil {
		log.WarnS(ctx, "Reviewer prompt: failed to load review for diff",
			err, "review_id", r.reviewID,
		)
	} else {
		diffContent = rev.DiffContent
		diffCommand = rev.DiffCommand
	}
	...
	if r.isMultiReview {
		return renderCoordinatorReviewPrompt(ctx, data)
	}

	return renderReviewPrompt(ctx, data)
}
...
func (r *reviewSubActor) hookStop(
	...
	for time.Now().Before(deadline) {
		...
		if len(msgs) > 0 {
			...
			// stored diff for re-review context.
			diffContent, diffCmd := "", ""
			if rev, err := r.store.GetReview(
				storeCtx, r.reviewID,
			); err == nil {
				diffContent = rev.DiffContent
				diffCmd = rev.DiffCommand
			}
			reason := formatMailAsReReviewPrompt(
				msgs, diffContent, diffCmd,
			)
...
// includes the feedback messages and the stored diff so Claude can
// re-review the code and emit an updated YAML review block without
// running git locally.
func formatMailAsReReviewPrompt(
	msgs []store.InboxMessage, diffContent, diffCmd string,
) string {
